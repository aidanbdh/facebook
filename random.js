function postRandom() {
  ...
  const selectProfiles = profiles.filter(function notUser(user) {
    return user !== currentUser
  });
  const randomProfile = profiles[Math.floor(Math.random() * selectProfiles.length)];
  const randomPoster = selectProfiles[Math.floor(Math.random() * selectProfiles.length)];
  ...
  for (let i = 0; i < randomPoster.followers.length; i++) {
