function postRandom() {
  ...
  randomProfile.posts.push(new PostObject(randomPoster,randomText));
  //Send to notifications of friends
  for (var i = 0; i < randomPoster.followers.length; i++) {
    randomPoster.followers[i].notifications.push(new PostNotification(randomText,randomPoster));
  };
  //Send to notifications of whoever's page the post is on
  if(randomPoster.followers.indexOf(randomProfile) === -1) {
    randomProfile.notifications.push(new PostNotification(randomText,randomPoster));
  };
  //Add a new post
